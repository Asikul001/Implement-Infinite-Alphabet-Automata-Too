...
// actual function
void put_data(const std::string &path, data &init,
    ...
    while (!file.eof() && std::getline(file, hold, '\n')) {
      if (init.automata_name == "ra" || init.automata_name == "RA") {
        if (hold[0] == '{') {
          ...
          transition_pair.ra_pair.insert(
              {{hold_data.first_state, hold_data.transaction},
               {hold_data.register_name, hold_data.next_state}});
        }
      } else {
        temp hold_data = get_data(hold, init.automata_name);
        transition_pair.dfa_nfa_pair.insert(
            {{hold_data.first_state, hold_data.transaction},
             hold_data.next_state});
      }
    }
    file.close();
  } else {
